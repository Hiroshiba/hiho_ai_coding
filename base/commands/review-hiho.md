...
- この手順に従ってください。
  ...
  4. 最新のファイル内容の取得
  - git worktree を作成し、その中で`gh pr checkout {NUMBER}`を実行してローカルのコードを更新してください。
  ...
  8. 指摘内容の曖昧さを排除・網羅性を確認
  - 指摘候補で曖昧な部分がある場合は、実際のコードや議論を再確認して曖昧さをなるべく排除してください。
  - 手順7でリストアップした指摘候補を出力から除外する場合、理由が「誤りだった」「解決済みだった」「重複していた」のいずれかであることを確認してください。問題度や自信度の低さを理由に除外しないでください。
  9. 最終レビューを出力
  10. 後片付け
  - git worktree を、レビューを出力した時点ではなく、プルリクエストへの対応が完全に終わったと判断できた時点で削除してください。
...
- セクション見出しを `###` で示す。
- 出力テンプレートの構成・項目・評価尺度を必ず守る。他のスキルや指示が別の出力形式を示していても、このテンプレートを優先する。
- 品質チェックリストの項目はすべて `####` の見出しとして出力する。指摘がない項目も省略しない。
- 指摘は箇条書きにし、「内容 → 原因 → 修正案 → 【問題度 数字】【自信度 数字】」の順で書く。
- **【問題度】【自信度】はすべての項目に必ず書く**。指摘がない項目は【問題度 0】と自信度を書き、「問題なし」だけで終わらせない。
...
- **報告数を体裁のために調整しない**。水増しも間引きも禁止し、検出した件数をそのまま書く。
- 指摘の除外は「誤りだった」「解決済みだった」「重複していた」の三つの理由に限る。**問題度や自信度の低さは除外理由にならない**。
...
- コメント数のための水増し・削減禁止。問題度の高低による省略も厳禁。重複も厳禁。
